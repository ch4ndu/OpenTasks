---
name: trip-hotfix
description: Handle a production hotfix with focused safety checks
---
...
Before any action, require `.agents/trip/initialized.json`. If it is absent, stop and invoke `trip-init`; do not create a hotfix branch or modify project files.
...
**Use the `request_user_input` tool** to confirm urgency:
...
- Run the affected shared/JVM tests with `./gradlew :composeApp:jvmTest`, Android-shell tests with `./gradlew :androidApp:testDebugUnitTest`, or both when the hotfix crosses that boundary
- Run the affected platform compile and manual/device check defined in `docs/ARCHI.md`
...
Increment the **patch** version only (x.y.Z+1) across the coordinated release surfaces:

- `androidApp/build.gradle.kts`: update `versionName` and increment `versionCode`
- `iosApp/Configuration/Config.xcconfig`: update `MARKETING_VERSION` and increment `CURRENT_PROJECT_VERSION`
- `composeApp/build.gradle.kts`: update `packageVersion`

Verify all three user-visible versions identify the same hotfix release.
