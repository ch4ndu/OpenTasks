...
Screen ViewModels expose `StateFlow` values derived from UseCase flows. Expensive filtering, grouping, formatting maps, and calendar projections run outside composables, commonly on `Dispatchers.Default`, and use `SharingStarted.WhileSubscribed(5000)`. Calendar renders consume one mode-tagged snapshot whose day, today marker, required projections, and hour labels share one source revision; unchanged raw day inputs reuse their day and row projections. Composables render state and forward user intent; they do not call repositories or DAOs.
...
External boundaries are validated before expensive work. File/share inputs use bounded byte reads and strict UTF-8, typed rejections, and stale-generation checks. The iOS share extension durably publishes bounded App Group envelopes for up to 24 hours without depending on opening the app. A serialized receiver reserves an active, mounted account/epoch ticket before claiming one disk record, revalidates the foreground/readiness generation before process publication, and keeps one task or ICS review lease until its editor/import flow ends. Resigning active invalidates in-flight tickets while an established review survives ordinary background/resume. Temporary PocketBase sessions and clients own one close/unregister path, while failed candidate setup leaves the active client usable. Coroutine cancellation remains cancellation across imports, attachments, sync, Settings, and export handoff. JVM Calendar processes are run through a timeout- and 16 MiB-output-bounded runner that cleans up children on timeout, cancellation, and overflow.
...
`androidApp` owns `OpenTasksApplication`, `MainActivity`, notification receivers, `SyncWorker`, and three Glance widget families (task list, calendar, and week). The application pre-warms Room and refreshes widgets after process start. WorkManager requests periodic maintenance approximately every two hours. The worker captures one active cache boundary, skips network sync in local-only mode, then revalidates that same live boundary once and holds the foreground mutation gate while the reminder, task-widget, calendar-widget, and week-widget maintenance steps run independently. Ordinary sync failure does not suppress local maintenance; authentication rejection, cancellation, or a stale boundary stops later effects.
...
The share extension accepts bounded shared content and, after explicit Save, stores one owner-only envelope in the `group.com.udnahc.opentasks` App Group. It reports that the item was saved for later review and does not require `extensionContext.open`. Pending records remain eligible for 24 hours and deterministic discovery orders them by creation time then nonce. A native lock serializes temporary, pending, and claimed states; interrupted claims are retired rather than replayed, and the app deletes and syncs a claimed envelope before ticketed publication to shared code. Launch, foreground activation, exact-nonce URLs, and every blocked-to-ready Compose transition converge on one coalesced scanner. The scanner requires an active app and mounted active-cache UI with no editor, modal, import, review, or process payload. Resigning active invalidates a reserved ticket and prevents stale publication without releasing an established task or ICS review lease; resume re-signals eligible pending intake. Task creation and confirmed ICS import each hold the single review lease until their terminal Save, Cancel, dismissal, or import result. Platform source code supplies iOS calendar, file, attachment, notification, and theme implementations.
