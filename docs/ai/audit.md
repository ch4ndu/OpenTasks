...
## Phase 5 dependency and lint policy

Dependency freshness is the normal goal. Update compatible catalog entries in small attributable batches and compile the affected path after each batch. Compose Multiplatform `1.11.1` and RichEditor `1.0.0` are now updated together because the stable RichEditor release requires the Compose 1.11.1 baseline; Material 3 remains on `1.11.0-alpha07`, the component version used by that Compose line. Gradle `9.4.1` and AGP `9.2.1` remain the empirically verified project pair: Kotlin `2.4.10`'s published KMP compatibility table lists support through AGP `9.1.0`, so AGP `9.3`/Gradle `9.5` are not adopted without a supported Kotlin line or separately authorized validation. Stable Android API 36 remains the compile/target ceiling because API 37 is beta. AndroidX/navigation/lifecycle/adaptive entries are kept only when their published metadata and affected compile remain compatible with API 36; a newer artifact that requires API 37 stays on its prior version with the exact metadata/compile evidence beside the pin. The narrow lint suppressions for the current-stable Gradle/AGP and Compose metadata checks are not a baseline. Preview dependencies remain pinned only where no compatible stable replacement exists. No broad lint baseline is permitted: source, resource, RTL, performance, accessibility, and dependency-scope warnings must be fixed or narrowly justified.
