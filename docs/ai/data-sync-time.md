...
- User-visible calendar/task/widget formatting goes through the domain `LocalizedDateTimeFormatter`. Stored pseudo-local millis are always formatted in UTC so the formatter cannot apply the device offset a second time; platform actuals supply locale and time-cycle preferences.
- Calendar and CSV imports persist a canonical external identity from the first 16 bytes of a platform SHA-256 digest. Within one batch, a canonical match does not consume the legacy alias; otherwise at most one row may consume a matching legacy alias so formerly colliding rows can import under their canonical identities.
...
- Windows/Linux token persistence fails closed unless `.opentasks` is a real
  owner-only directory and the token is a real owner-only regular file. Use
  POSIX `0700`/`0600`, or apply and verify an equivalent single-owner ACL when
  POSIX attributes are unavailable. Secure and verify an empty temporary file
  before writing credentials; validate without following links before every
  read, replacement, or delete. Never retain raw filesystem/JSON failures in a
  `SecureTokenStoreException` cause.
...
- Scheduled maintenance captures one active-cache boundary before optional sync. Local-only mode makes no network call. After sync success or ordinary failure, it revalidates that same live boundary once and holds the foreground mutation gate while the reminder, task-widget, calendar-widget, and week-widget steps run independently. Authentication rejection, cancellation, or stale-boundary rejection stops later effects; ordinary local-step failures do not. Preserve the original sync failure and attach later maintenance failures as suppressed evidence.
...
- Attachment stores encode before allocating paths, durably lease the image/thumbnail pair in `attachment_file_cleanup` before the first write, and compensate cancelled or failed result handoff in `NonCancellable`. Attachment downloads then conditionally install the paths after a writer-boundary reread; that transaction journals differing predecessor paths before publishing the successor. Network and private-file reads, including upload and seed storage reads, enforce `AttachmentFilePolicy.MAX_UPLOAD_BYTES`; a newer local edit or tombstone keeps its metadata and leaves the losing downloaded artifact durably retryable. An equal-timestamp missing-file retry may install only when the exact captured active-row snapshot is unchanged and the remote canonical metadata and filename still match; filesystem/network work stays outside the writer transaction. A same-file pull shortcut requires a healthy synced row with both local files present, and a newer remote row discovered during push is deferred to pull reconciliation so blank remote projections cannot replace owned paths.
- Active attachment upload accepts only the supported task/image identity with a present, active task parent. A parent without `pbId` defers normal upload until its dependency is synced; missing, deleted, or unsupported parents fail the attachment push without deleting the child row or files.
...
- The exact pristine installation-created Inbox placeholder may yield to the matching remote category at an equal timestamp during account activation. No edited, synced, deleted, timestamped, or remotely identified category receives this exception.
...
- Normal synced deletes are pushed as durable PocketBase tombstones with `isDeleted = true`; do not hard-delete server rows for app deletes. Before deleting any `pbId == null` tombstone locally, perform the owner-scoped remote `localId` lookup. Adopt a found identity and run the normal guarded last-write-wins path. Only a valid empty result may authorize writer-transaction deletion after rechecking the unchanged unsynced tombstone, null identity, timestamp, and child guard; lookup/auth/network/malformed/cancellation failures retain the row. Task or tag parents with durable remote children remain push candidates. The only server hard-delete exception is the confirmed owner-scoped authoritative replacement executor.
- Task deletion is a single Room writer-boundary graph mutation: tombstone active task-tag links and task attachments before tombstoning the task, then invoke the shared attachment cleanup owner only after commit. A failed database mutation leaves the complete graph and files unchanged; an uncertain file cleanup leaves the durable tombstone paths or cleanup-ledger entries recoverable.
- Attachment tombstone cleanup retains both file paths until both files are proven absent. Startup and general retries clear proven-clean paths but retain the tombstone row; normal non-seed sync may hard-delete a pathless tombstone whose `pbId` is still null only after an owner-scoped local-ID lookup proves remote absence. A matching remote row is adopted first so a committed create whose local `pbId` write was lost still receives the deletion tombstone.
- Cleanup-ledger entries are path-exact and account-owned. A referenced path drops only its ledger entry; an unreferenced path is released only after deletion and an authoritative absence check. Account cache replacement deletes ledger rows in the Room reset transaction because the durable `FILES_PENDING` transition then owns proof that the complete attachment directory is absent.
