---
name: trip-research
description: Investigate a question without directly producing production code
---
...
Before any action, require `.agents/trip/initialized.json`. If it is absent, stop and invoke `trip-init`; do not start the research workflow.
...
**Use the `request_user_input` tool** to confirm the thinking level:
...
Present the research plan summary to the user, then **use the `request_user_input` tool** with two questions:
...
**If "Adjust"**: Modify the plan based on user feedback, then re-present using `request_user_input`.
...
## Step 4b: Terra Cross-Check

For **decision-grade findings** — architecture recommendations, technology choices, anything the user will build on (typically compute level `think hard` and above) — red-team the draft conclusion with Terra's private `ask` phase before presenting. Skip for quick lookups.

```bash
python3 .agents/trip/bin/launch_runtime.py run \
    --module ask --phase start --role terra --target <topic-label> --round 1 \
    --prompt "Here is my draft recommendation: <summary + key rationale>. Red-team it: what am I missing, what would you choose instead, and why?"
```

Follow up in the same verified thread (`--phase follow-up --resume`) if the answer raises points worth probing. Then:
...
Then **use the `request_user_input` tool**:
...
If research concludes that implementation should proceed, **use the `request_user_input` tool**:
