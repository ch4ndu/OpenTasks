---
name: trip-compact
description: Compact ARCHI.md while preserving essential architecture context
---
...
Before any action, require `.agents/trip/initialized.json`. If it is absent, stop and invoke `trip-init`; do not modify architecture documentation.
...
```bash
bash .agents/trip/bin/count-tokens.sh docs/ARCHI.md
```
...
**If token count > 20,000**, report the assessment to the user, then **use the `request_user_input` tool**:
...
**If token count <= 20,000**, report to the user, then **use the `request_user_input` tool**:
...
Present the compaction results to the user, then **use the `request_user_input` tool**:
