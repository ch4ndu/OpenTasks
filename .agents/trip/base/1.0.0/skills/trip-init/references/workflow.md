...
**Use the `request_user_input` tool** to present the user with a structured choice:
...
- **If "Request changes"**: Make the requested modifications, then re-present for validation using `request_user_input` again
- **If "Add sections"**: Add them, then re-present for validation using `request_user_input` again
...
The week Init is run becomes **Week 1** of the project. Capture the anchor as **epoch seconds of the Monday of the current week** — the week formula in `trip-3-release` Step 1 uses plain `date +%s` arithmetic, portable across BSD/macOS and GNU/Linux:
...
print(int(datetime.datetime.combine(monday, datetime.time()).timestamp()))
EOF
```

Then replace the `[WEEK_ANCHOR_EPOCH]` placeholder in `trip-3-release` Step 1 with that number. The formula counts elapsed weeks from that fixed moment, so it works across year boundaries indefinitely.
...
- **Use the `request_user_input` tool** with multiple questions to customize tutorial generation:
...
**IMPORTANT — Renumber subsequent steps**: After inserting the Tutorial as Step 8 (right after Step 7: Architecture Update), renumber the steps that follow:
...
Also delete the explanatory HTML comment under the `[TUTORIAL_STEP]` marker (in both the enabled and disabled cases).

**D. Terra Review Test Commands**
...
Also replace the command placeholders in `trip-review/checklist.md`'s Approval Gate with the same commands.

**E. Main Branch**

Replace the `[MAIN_BRANCH]` placeholder with the repository's default branch name (commonly `main`; check `git symbolic-ref --short HEAD` on a fresh clone or the repo's configured default). It appears in:

- `trip-3-release` Step 11 (fast-forward merge)
- `trip-hotfix` Steps 2 and 8

**F. Integration Impact Check**

Replace the `[ADAPT_TO_PROJECT: Integration Impact]` marker (and its explanatory comment) in `trip-2-implement`'s Testing Gate step 3 with the project's integration/E2E verification guidance: which changes require the full verification gate, which require manual/device/emulator/staging checks, and which external contracts must be exercised against a real service. Keep the "Docs-only changes skip this" bullet.
...
Update the Checklist section in `cr-template.md` to list the **actual section names** from the adapted `checklist.md`. The template ships with generic section names (1-6); after adapting the checklist, the template's section list must match. Delete the trip-init instruction comment beneath the list.
...
- [ ] **TRIP skills updated** (Phase 6):
  ...
  - [ ] `trip-2-implement`: `[ADAPT_TO_PROJECT: Integration Impact]` marker replaced with project integration/E2E guidance
  - [ ] `trip-3-release`: `[VERSION_FILE]` placeholder replaced
  - [ ] `trip-3-release`: `[WEEK_ANCHOR_EPOCH]` placeholder replaced
  - [ ] `trip-3-release` + `trip-hotfix`: `[MAIN_BRANCH]` placeholder replaced
  ...
  - [ ] `trip-review/checklist.md`: `[ADAPT_TO_PROJECT]` marker replaced with project-specific checklist sections; Approval Gate command placeholders filled
